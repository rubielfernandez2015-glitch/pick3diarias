...
- LEE COLUMNA POR COLUMNA: si la hoja tiene varias columnas (tabla/libreta), procesa una columna COMPLETA de arriba a abajo y solo despues pasa a la siguiente. Cada columna es INDEPENDIENTE: la llave va PEGADA a sus numeros y el monto PEGADO a la llave, todo dentro de la MISMA columna. NUNCA tomes el monto de una columna para los numeros de otra; nunca mezcles columnas.
- NO TE SALTES NINGUNA FILA: recorre la columna renglon por renglon de arriba a abajo y transcribe TODOS los numeros que veas, aunque esten tenues, apretados o pequenos. NO dejes numeros sin transcribir en medio de la lista. Es preferible incluir un numero dudoso (con " ?") que omitirlo. Cuenta las filas para no saltarte ninguna.
...
3) GRUPO POR LLAVE / RAYA / FLECHA (clave en hojas-libreta manuscritas): los numeros se listan en VERTICAL (uno debajo de otro) y una LLAVE (corchete } ), raya o flecha los AGRUPA; el monto del grupo se escribe a la DERECHA de la llave (mas o menos a la mitad de la lista) y aplica a CADA numero del grupo POR IGUAL. Ej: una llave que abarca 03,04,14,23,86 con "50" a la derecha -> 03-50,04-50,14-50,23-50,86-50. Puede haber VARIAS llaves seguidas en una columna, cada una con su propio monto (ej. el grupo siguiente del 24 al 94 con "20"). Una llave puede agrupar una lista de numeros sueltos o un rango (60 al 69 con llave de 100 -> 60-100,...,69-100). Un numero SIN llave lleva su monto pegado: "25-100" -> 25-100. Los NOMBRES entre grupos separan clientes: IGNORALOS. Tambien sirve "de 50" o un monto escrito una sola vez para el grupo.
